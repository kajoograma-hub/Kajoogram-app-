...
const PlatformPage: React.FC<PlatformPageProps> = ({ isLoggedIn = false }) => {
  const { name } = useParams<{ name: string }>();
  const navigate = useNavigate();
  const { products } = useProductContext();

  // Find the platform object based on the URL parameter
  const platform = PLATFORMS.find(p => p.name === name);

  // Filter products that belong to this platform
  const platformProducts = useMemo(() => {
    if (!platform) return [];
    return products.filter(p => p.platform === platform.name);
  }, [platform, products]);

  if (!platform) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center space-y-4">
        <h2 className="text-2xl font-serif font-bold text-slate-300">Platform Not Found</h2>
        <button 
          onClick={() => navigate(-1)}
      ...
      <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl px-6 py-4 flex items-center space-x-4 border-b border-slate-50 transition-all">
        ...
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 rounded-full overflow-hidden border border-slate-100">
             <img src={platform.image} className="w-full h-full object-cover" alt={platform.name} />
          </div>
          <h1 className="text-xl font-serif font-bold text-slate-800 tracking-tight">{platform.name}</h1>
        </div>
      </div>

      <div className="p-6">
        <div className="mb-8 flex items-center justify-between">
           <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{platformProducts.length} Products</span>
        ...
        {platformProducts.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 animate-in zoom-in duration-500">
            ...
            <p className="text-sm text-slate-400 max-w-[200px]">
              This platform hasn't added any products yet.
            </p>
          </div>
        )}
      </div>
